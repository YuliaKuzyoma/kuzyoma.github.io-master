class SimpleDataTableWidget{
    ...
    renderWidget()
    {
        ...
        let tableHtml = $('<table class="table table-striped" style="table-layout: fixed; width: 50%;">');
        //headers
        let header = $('<tr>');
        ...
        for(let i = firstDisplayPage;i<lastDisplayPage;i++)
        {
            ...
            if(i === this.currentPage)
                enable = 'disabled';
            let button = $('<button ' + enable +' class="btn btn-sm btn-default" style="margin: 2px 10px 10px 0; width: 40px; height: 40px;">').text(i+1);
        ...
        controlPanel.append($('<button  type="button" class="btn btn-default" data-toggle="modal" data-target="#addRowModal">').text('Create'));
        controlPanel.append($('<button class="btn btn-default" data-toggle="modal" data-target="#deleteRowsModal" style=" margin-left: 10px ;">').text('Delete Selected'));
        controlPanel.append($('<button class="btn btn-default   " style=" margin-left: 10px ;">').text('Refresh').click(function (objName) {window[objName].refresh();}));
